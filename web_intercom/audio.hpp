...
typedef void(*audio_send_fcn_t)(const uint8_t*, size_t);
...
class IntercomAudio{
  private:  
    ...
    audio_send_fcn_t audio_send_fcn;
  public:
    IntercomAudio(audio_send_fcn_t audio_send_fcn){
        this->audio_send_fcn = audio_send_fcn;
    ...
    void recv_from_ws(const unsigned char* data, size_t len) {
      static buf8_t buf8;
      if(len!=NUM_SAMPLES){
    ...
    void send_to_ws() {
        ...
        if(buf8){
          this->audio_send_fcn(buf8->buf, NUM_SAMPLES);
        }
    }
};
